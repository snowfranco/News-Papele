...
Markers are live wherever manifold persists the backing ids, which is every editorial surface: the lede (title and why-this-leads, from `lede.itemIds`), the emerging cards (`emerging[].itemIds`), the theme map's detail panel and the Position desk's progress line (`themes.item_ids`), and Start Here rows (each row is its own article link plus a Feeds deep-link). manifold writes at least one id per emerging entry and per theme, enforced by a deterministic gate check (`citation-ids-present`, manifold/src/gate.ts) so this cannot regress silently; the contract lives in src/types.ts + src/schemas.ts and manifold/src/contract.ts together, and themes gained an `item_ids` column (supabase/migrations/20260816000000_theme_item_ids.sql).

Where ids are genuinely absent, the marker renders in a disabled "no sources recorded" state rather than hiding, so the gap is visible and nothing is invented. Two cases remain:

- Editions and themes written before the ids were persisted carry none, and degrade to the disabled marker until manifold's next pass rewrites them.
- **[CONTRACT-NOTE]** manifold's devil's-advocate counter-argument is not persisted or rendered yet (src/views/PositionDeskView.tsx); when manifold writes its pushback it should carry cited ids like every other claim. That is a separate follow-up in the manifold code.
