...
Markers are live wherever manifold persists the backing ids, which is every editorial surface: the lede (title and why-this-leads, from `lede.itemIds`), the emerging cards (`emerging[].itemIds`), the theme map's detail panel and the Position desk's progress line (`themes.item_ids`), Start Here rows (each row is its own article link plus a Feeds deep-link), and manifold's devil's-advocate reply on each column at the Position Desk (`manifold_replies.item_ids`, src/views/PositionDeskView.tsx). manifold writes at least one id per emerging entry, per theme, and per reply, enforced by deterministic gate checks (`citation-ids-present` for editorial writes and `reply-citation-ids-present` for the devils-advocate pass, manifold/src/gate.ts) so this cannot regress silently; the contract lives in src/types.ts + src/schemas.ts and manifold/src/contract.ts together, and themes gained an `item_ids` column (supabase/migrations/20260816000000_theme_item_ids.sql) alongside the manifold_replies table (supabase/migrations/20260816010000_manifold_replies.sql).

Where ids are genuinely absent, the marker renders in a disabled "no sources recorded" state rather than hiding, so the gap is visible and nothing is invented. One case remains: editions and themes written before the ids were persisted carry none, and degrade to the disabled marker until manifold's next pass rewrites them.
